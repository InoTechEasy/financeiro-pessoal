...
import { FormularioTipoLancamento } from './FormularioTipoLancamento';
import { ModalLancamento } from '../Lancamentos/ModalLancamento';

export const GerenciadorTiposLancamentos: React.FC = () => {
  ...
  useEffect(() => {
    const carregarTipos = async () => {
      try {
        const dados = await tiposLancamentosService.listar();
        setTipos(dados);
      } catch (error) {
  ...
  return (
    <div>
      ...
      <div className="space-y-2">
        {tipos.map((tipo) => (
          <div key={tipo.id_tipo_lancamento} className="bg-gray-50 p-3 rounded-lg flex justify-between items-center">
            <div>
      ...
      <ModalLancamento isOpen={showModal} onClose={() => setShowModal(false)}>
        <FormularioTipoLancamento onSuccess={handleSuccess} editData={editData} />
      </ModalLancamento>
    </div>
  );
};
